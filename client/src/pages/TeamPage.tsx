...
export default function TeamPage() {
  ...
  return <div className="mx-auto max-w-5xl space-y-6">
    <section>
      ...
      <p className="page-lede mt-2 max-w-3xl">El Contador puede consultar los usuarios. El Administrador asigna roles, incluyendo participantes adicionales para cargar gastos. Cada persona debe iniciar sesión una vez para aparecer en este listado.</p>
    </section>
      ...
      <CardContent className="p-0">
        <div className="overflow-x-auto">
          <table className="w-full min-w-[720px] text-left text-sm">
            <thead className="bg-[#f7f9fe] text-xs font-bold uppercase tracking-wide text-[#71809a]"><tr><th className="px-5 py-3">Nombre operativo</th><th className="px-5 py-3">Correo</th><th className="px-5 py-3">Rol del proyecto</th><th className="px-5 py-3">Último acceso</th><th className="px-5 py-3">Administración</th></tr></thead>
            <tbody className="divide-y divide-[#e8edf7]">
              {users.data?.map(member => <tr key={member.id} className="transition-colors hover:bg-white/65"><td className="px-5 py-4 font-medium text-[#1c2942]"><p>{member.displayName || member.name || "Sin nombre"}</p>{member.displayName && member.name && member.displayName !== member.name ? <p className="mt-0.5 text-xs font-normal text-[#71809a]">Cuenta: {member.name}</p> : null}</td><td className="px-5 py-4 text-[#65718a]">{member.email || "—"}</td><td className="px-5 py-4"><span className="inline-flex rounded-full border border-white/75 bg-white/65 px-2.5 py-1 text-xs font-semibold text-[#53617a] shadow-[inset_0_1px_0_rgba(255,255,255,0.8)]">{USER_ROLE_LABELS[member.role]}</span></td><td className="px-5 py-4 text-[#65718a]">{formatDate(member.lastSignedIn)}</td><td className="px-5 py-4">{canManage ? <select value={member.role} onChange={event => applyRole(member.id, event.target.value as "socio_1" | "socio_2" | "participante" | "contador" | "admin" | "user")} disabled={assignRole.isPending} className="glass-input h-9 min-w-[190px] px-3 text-xs"><option value="user">Usuario sin asignar</option><option value="socio_1">Socio 1 (Ing. Angomas)</option><option value="socio_2">Socio 2 (Ing. Johan)</option><option value="participante">Participante</option><option value="contador">Contador</option><option value="admin">Administrador</option></select> : <span className="inline-flex items-center gap-1 text-xs text-[#8b96ab]"><ShieldCheck className="h-3.5 w-3.5" />Solo lectura</span>}</td></tr>)}
            </tbody>
          </table>
        </div>
